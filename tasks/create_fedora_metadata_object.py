...
from fedora_parts_builder import PartsBuilder
...
class Task( object ):
    ...
    def create_fedora_metadata_object( self,
        ...
        try:
            ...
            print u'- coll_obj instantiated...(TURNED OFF)'
            #
            #Get/reserve a pid
            #Note: after obtaining pid, new_obj.pid is a value, not a reference
            # new_pid = new_obj.pid()
            print u'- new_obj.pid obtained: %s (TURNED OFF)' % new_pid
            #
            #Assign collection object
            # new_obj.owning_collection = coll_obj
            print u'- collection object assigned (TURNED OFF)'
            #
            1/0
