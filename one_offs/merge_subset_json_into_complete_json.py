...
class Merger( object ):
    ...
    def __init__( self ):
        self.SOURCE_FULL_JSON_METADATA_PATH = unicode( os.environ[u'BELL_ONEOFF__SOURCE_FULL_JSON_METADATA_PATH'] )
        self.SOURCE_SUBSET_JSON_METADATA_PATH = unicode( os.environ[u'BELL_ONEOFF__SOURCE_SUBSET_JSON_METADATA_PATH'] )
        self.OUTPUT_PATH = unicode( os.environ[u'BELL_ONEOFF__OUTPUT_FULL_JSON_METADATA_PATH'] )

    def merge_data( self ):
        """ Merges subset into full set; outputs new full set. """
        ( initial_full_dct, subset_dct ) = self.loadup()
        key_val_lst = sorted( subset_dct[u'items'].items() )
        for (accession_num, item_dct) in key_val_lst:
            initial_full_dct[u'items'][accession_num] = item_dct
    ...
    def loadup( self ):
        ...
        logger.debug( u'in one_offs.merge_subset_json_into_complete_json.Merger.loadup(); metadata_dct item_count, `%s`' % len(metadata_dct[u'items']) )
        logger.debug( u'in one_offs.merge_subset_json_into_complete_json.Merger.loadup(); subset_dct item_count, `%s`' % len(subset_dct[u'items']) )
